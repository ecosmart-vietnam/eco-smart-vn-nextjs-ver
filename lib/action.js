...
export async function login(username, password) {
  try {
    const { data, headers } = await http.post(`${origin}/api/login`, { username, password });
    return data;
  } catch (e) {
    console.log(e);
  }
...
export async function logout() {
  try {
    const { data, headers } = await http.post(`${origin}/api/logout`, {});
    return data;
  } catch (e) {
    console.log(e);
  }
...
export async function refresh() {
  try {
    const { data } = await http.post(`${origin}/api/refresh`, {});
